...
`GF2n` `Fintype` and `Field` scaffolding plus the large-degree
`GF2nPoly` quotient-field bridge layered on top of the executable
`HexGF2` arithmetic surface.
-/
