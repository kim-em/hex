...
/-!
Deterministic core conformance checks for `HexMatrix`.
...
- Covered properties:
  ...
  - Swapping distinct rows negates the determinant, while a self-swap leaves the matrix unchanged.
  - Scaling a row multiplies the determinant by the same scalar, including the zero-scalar edge case.
  - Adding a multiple of one row to another preserves the determinant, including the zero-coefficient edge case.
- Covered edge cases:
